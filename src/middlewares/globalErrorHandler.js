...
export const globalErrorHandler = (err, req, res, next) => {
    if (process.env.NODE_ENV === 'development') {
        ...
        if (error.code === 11000) error = handleDuplicateFieldsDB(error, next, req)
        if (error.name === 'ValidationError') error = handleValidationErrorDB(error, next, req)
        if (error.name === 'JsonWebTokenError') error = handleJWTError(error, next, req)
        if (error.name === 'TokenExpiredError') error = handleJWTExpiredError(error, next, req)
