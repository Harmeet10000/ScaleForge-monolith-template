...
import * as openFGAService from '../services/openFGAService.js';
...
export const projectExample = async () => {
  try {
    ...
    const orgMemberCanView = await policyManager.checkProjectAccess(orgMemberId, projectId, 'org_viewer');
...
export const documentExample = async () => {
  try {
    ...
    const projectMemberCanView = await policyManager.checkDocumentAccess(projectMemberId, documentId, 'project_viewer');
...
export const ownershipTransferExample = async () => {
  try {
    ...
    logger.info(`✅ Transferred ownership of project ${projectId} from ${currentOwnerId} to ${newOwnerId}`);
    ...
    const oldOwnerCanOwn = await policyManager.checkProjectAccess(currentOwnerId, projectId, 'owner');
...
export const lowLevelExample = async () => {
  try {
    ...
    const hasCustomAccess = await openFGAService.check(userId, 'custom_relation', resourceId, 'custom_type');
