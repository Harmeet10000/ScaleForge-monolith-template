...
export const findUserByEmailAddress = async (emailAddress, select = '') => await User.findOne({
        emailAddress
    }).select(select);

export const findUserByConfirmationTokenAndCode = async (token, code) => await User.findOne({
        'accountConfirmation.token': token,
        'accountConfirmation.code': code
    });

export const findByResetToken = async (token) => await User.findOne({
